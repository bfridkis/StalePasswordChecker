...
$checkDate = $outputMode = $fileName = $OUSpecific = $OUToSearch = $searchLevel = $helpRequest = $null

$admins = "srvWMI", "srvHSI", "srvD4K", "CPMAdministrator", "ACMAdministrator", "srvWUG"
...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        ...
        $userInfo | select-object @{n='Name';e={$_.Name}}, PasswordLastSet, 
                        @{n ='Is_Admin' ; e = { $(if ($_.description -like "*administrator*" -or $_.Name -in $admins) {'Yes'} else {'No'})}}, 
                        @{n = 'Is_Compliant' ; e = { $(if ($_.description -like "*administrator*" -or $_.Name -in $admins) {
                                                          if((New-TimeSpan -Start $_.PasswordLastSet -End $checkDate.AddDays(-90)).Days -lt 90) {
                                                            ...
                                                            if((New-TimeSpan -Start $_.PasswordLastSet -End $checkDate.AddDays(-365)).Days -lt 365) {
                                                                "YES"
                                                            }
                                                            else { "NO" }
                                                        }
                        ...
                        @{n = 'Number of Days Expired (+) or Until Expiration(-)' ; e = { $(if ($_.description -like "*administrator*" -or $_.Name -in $admins) {
    ...
    write-output "`n`n** Note: The 'IsAdmin' attribute in the result set is based on the 'description' property of Get-Aduser cmdlet."
    write-output "**       (More specifically, a user is flagged as admin if his or her description field contains the string 'administrator'.)"
