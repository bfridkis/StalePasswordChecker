...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope Subtree | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope $searchLevel | Where-Object { $_.Name -ne "Roy Lapeyronnie" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        $userInfo | select-object @{n='Name';e={$_.Name}}, PasswordLastSet, LastLogonDate,
        ...
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope Subtree | Where-Object { $_.Name -ne "Roy Lapeyronnie" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope $searchLevel | Where-Object { $_.Name -ne "Roy Lapeyronnie" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null } 
        $userInfo | select-object @{n='Name';e={$_.Name}}, PasswordLastSet, LastLogonDate,
                        @{n ='Is_Admin' ; e = { $(if ($_.memberof -like "*Domain Admins*") {'Yes'} else {'No'})}}, 
                        @{n = 'Is_Compliant' ; e = { $(if ($_.memberof -like "*Domain Admins*") {
