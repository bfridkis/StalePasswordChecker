...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True' -and Description -notlike '*Exception*'" -Property PasswordLastSet, Description, MemberOf, Enabled -SearchScope Subtree <#| Where-Object {$_.Enabled -ne $null}#> | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True' -and Description -notlike '*Exception*'" -Property PasswordLastSet, Description, MemberOf, Enabled -SearchScope $searchLevel <#| Where-Object {$_.Enabled -ne $null}#> | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
