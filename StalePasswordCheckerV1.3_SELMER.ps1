...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True' -and Description -notlike '*Exception*'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope Subtree | Where-Object { $_.Name -ne "Roy Lapeyronnie" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True' -and Description -notlike '*Exception*'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope $searchLevel | Where-Object { $_.Name -ne "Roy Lapeyronnie" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
