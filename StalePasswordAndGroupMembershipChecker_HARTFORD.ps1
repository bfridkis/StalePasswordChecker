...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        if ($OUSpecific -eq "N") { 
            get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate, PasswordNeverExpires -SearchScope Subtree | 
            Where-Object { !$_.Description -or $_.Description -notlike "*Exception*" } | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True' -and Description -notlike '*Exception*'" -Property PasswordLastSet, Description, MemberOf, Enabled, LastLogonDate -SearchScope $searchLevel | 
               Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null 
        }
        $userInfo | select-object @{n='Name';e={$_.Name}}, SamAccountName, PasswordLastSet, LastLogonDate, 
                        @{n = 'Days Since Last Logon' ; e = { (New-TimeSpan -Start $_.LastLogonDate -End $(Get-Date)).Days }},
        ...
        $userInfo | 
                     ...
                     select-object @{n='Name';e={$_.Name}}, SamAccountName, PasswordLastSet, LastLogonDate, 
                        @{n = 'Days Since Last Logon' ; e = { (New-TimeSpan -Start $_.LastLogonDate -End $(Get-Date)).Days }},
                        PasswordNeverExpires,
                        @{n ='Is_Admin' ; e = { $(if ($_.memberof -like "*Domain Admins*") {'Yes'} else {'No'})}}, 
                        @{n = 'Is_Compliant' ; e = { $(if ($_.memberof -like "*Domain Admins*") {
