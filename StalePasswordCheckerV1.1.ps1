...
if ($outputMode -eq 1) { 
    if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet, description -SearchScope Subtree | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
    else { get-aduser -Filter "Enabled -eq 'True'" -SearchBase $OUToSearch -Property PasswordLastSet, description -SearchScope $searchLevel | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
