...
$domainAdmins = get-adgroupmember -Identity "domain admins" -Recursive | select-object name

if ($outputMode -eq 1) { 
    if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property PasswordLastSet -SearchScope Subtree | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
    else { get-aduser -Filter "Enabled -eq 'True'" -SearchBase $OUToSearch -Property PasswordLastSet -SearchScope $searchLevel | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
    $userInfo | select-object @{n='Name';e={$_.Name}}, PasswordLastSet, 
                    @{n ='IsAdmin' ; e = { $(if ($domainAdmins.Name -contains $_.name) {'Yes'} else {'No'})}},  
                    @{n = 'Number of Days Expired' ; e = { $(if ($domainAdmins.Name -contains $_.name) {
    ...
    where-object {($_.IsAdmin -eq "Yes" -and $_.PasswordLastSet -lt $checkDate.adddays(-90)) -or 
                    ($_.IsAdmin -eq "No" -and $_.PasswordLastSet -lt $checkDate.adddays(-365))} |
    sort-object -Descending 'Number of Days Expired' -OutVariable Export | Format-Table
}
...
# https://stackoverflow.com/questions/50788152/read-host-always-ends-in-a-colon# https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_foreach?view=powershell-7
# https://stackoverflow.com/questions/27175137/powershellv2-remove-last-x-characters-from-a-string