...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        ...
        $userInfo | select-object @{n='Name';e={$_.Name}}, PasswordLastSet, 
                        @{n ='Is_Admin' ; e = { $(if ($domainAdmins.Name -contains $_.name) {'Yes'} else {'No'})}}, 
                        @{n = 'Is_Compliant' ; e = { $(if ($domainAdmins.Name -contains $_.name) {
                       ...
                       @{n = 'Number of Days Expired (+) or Until Expiration(-)' ; e = { $(if ($domainAdmins.Name -contains $_.name) {
    ...
    write-output "`n`n** Note: The 'IsAdmin' attribute is based on membership in the 'domain admins' group."
